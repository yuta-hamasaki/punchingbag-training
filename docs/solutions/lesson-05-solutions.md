...
pause: (nowMs) => {
  const timer = get();
  if (!(timer.state === "active" || timer.state === "resting") || timer.deadlineMs === null) return;
  set({ state: "paused", pausedRemainingMs: Math.max(0, timer.deadlineMs - nowMs), deadlineMs: null });
},
```
...
resume: (nowMs) => {
  const timer = get();
  if (timer.state !== "paused" || timer.pausedRemainingMs === null) return;
  set({ state: "active", deadlineMs: nowMs + timer.pausedRemainingMs, pausedRemainingMs: null });
},
```
